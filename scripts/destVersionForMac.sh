...
install_depends() {
    ...
    local missing_deps=()
    for cmd in wget curl git gh jq; do
...
main() {
    ...
    if [ "${VERSION}-mac" = "$LATEST_VERSION" ]; then
        if [ "$NOW_SUM256" = "$LATEST_SUM256" ]; then
            ...
            # 二次下载验证
            local TEMP_FILE="${TEMP_PATH}/temp/WeChatMac_verify.dmg"
            echo_color "yellow" "开始二次下载验证..."
            wget -q --continue "$DOWNLOAD_LINK" -O "$TEMP_FILE" || {
            ...
            if [ "$NOW_SUM256" = "$VERIFY_SUM256" ]; then
                ...
                # 创建一个 issue 来通知管理员
                gh issue create \
                    --title "检测到相同版本号但哈希值变化 [${VERSION}-mac]" \
                    --body "**版本信息**
- 版本号: ${VERSION}-mac
- 原始哈希值: ${LATEST_SUM256}
- 新哈希值: ${NOW_SUM256}
...
二次下载验证已通过，新旧哈希值不同，可能是官方更新了安装包。" || echo_color "yellow" "Issue 创建失败，继续执行..."
    ...
    rm -f "${TEMP_PATH}/temp/WeChatMac_verify.dmg" 2>/dev/null || true
}
